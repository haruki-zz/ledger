...
- 不在范围：搜索/筛选功能、色盲友好设计、自动备份（仅手动 CSV 导出）、Android/Web 客户端、实体账户管理（仅记录用户输入）。
...
- 预算结余全额结转到下月总预算，通过 budget_rollovers 记录。
...
## 12. 风险与待确认
- 若 Supabase 开发阻塞需回退 iCloud Numbers，需确认迁移成本与同步策略。
- 是否需要附件/照片、地理位置、预算备注等扩展字段（当前未包含）。
- 导出范围与格式细节（当前定义为手动 CSV，需确认入口位置与筛选维度）。
- 账户体系简化后，后续若引入实体账户/账单日可能影响数据模型与转账规则。
